...
How much filtering that is, measured on 2026-09-03: **15,087** repositories carry a dsh discovery topic and **14,688** of them — 97% — have been opened, read, and decided. 2,352 were rejected, **2,202** of those for having no install path at any depth: no `dsh` manifest in `package.json`, no dsh dependency, no `SKILL.md`. They carry the topic and nothing else. Every rejection is published with its reason and a recheck date in [`data/rejected.json`](data/rejected.json).

That is the number worth comparing. A topic count says how many people typed a tag. **14,688 of 15,087** says how many repositories somebody actually opened.
